...
flags.DEFINE_integer('iperf_timeout', None,
                     ...
                     lower_bound=1)
flags.DEFINE_string('test_type2', None, 'How to run the test')

FLAGS = flags.FLAGS
FLAGS.test_type2 = ('vm', 'contain')
...
def Prepare(benchmark_spec):
  ...
  for vm in vms:
    vm.Install('docker')
    if FLAGS.test_type2 == 'contain':
        vm.RemoteCommand('sudo docker pull ubuntu')
        c_cmd = ('sudo docker run --name test ubuntu')
        queen, _ = vm.RemoteCommand(c_cmd, should_log=True)
        vm.RemoteCommand('sudo apt-get update')
        vm.RemoteCommand('sudo apt-get install iperf')
        vm.RemoteCommand('cd /usr/bin')
