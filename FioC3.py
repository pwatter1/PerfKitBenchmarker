...
flags.DEFINE_string('test_type1', None, 'How to run the test')
FLAGS.test_type1 = ('vm', 'contain')
...
def Prepare(benchmark_spec):
  ...
  vm.Install('fio')
  vm.Install('docker')
  if FLAGS.test_type1 == 'contain':
      vm.RemoteCommand('sudo docker run --name test ubuntu')
      vm.RemoteCommand('sudo apt-get update')
      vm.RemoteCommand('sudo apt-get install -y fio')
