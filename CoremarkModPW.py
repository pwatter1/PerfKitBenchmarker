...
def Prepare(benchmark_spec):
  ...
  vm.Install('docker')
  vm.RemoteCommand('sudo docker run -i -t --name test ubuntu /bin/bash')
  vm.RemoteCommand('apt-get update')
  vm.RemoteCommand('apt-get install git')
  vm.RemoteCommand('git clone https://github.com/pwatter1/koolaid')
  vm.RemoteCommand('cd koolaid')
  try:
  ...
  """
  c.RemoteCommand('sed -i -e "s/LFLAGS_END += -lrt/LFLAGS_END += -lrt '
                   '-lpthread/g" %s/%s' % (COREMARK_DIR, COREMARK_BUILDFILE))
  """
  vm.RemoteCommand('mkdir 64 && cp linux64/* 64/')
  vm.RemoteCommand('make PORT_DIR=64')

  """
  vm.RemoteCommand('./coremark.exe')
  """



def Run(benchmark_spec):
  """Run Coremark on the target container.
  Args:
    benchmark_spec: The benchmark specification. Contains all data that is
        required to run the benchmark.
  Returns:
  ...
  metadata = {'num_cpus': c.num_cpus}
  metadata.update(c.GetMachineTypeDict())
...
def Cleanup(benchmark_spec):
  ...
  vms = benchmark_spec.vms
  vm = vms[0]
  vm.RemoteCommand('stop test')
  vm.RemoteCommand('rm test')
